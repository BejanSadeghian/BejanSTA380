...
DTM.train = DocumentTermMatrix(my_corpus.train, control= list(weighting = function(x) weightTfIdf(x, normalize = TRUE)))
DTM.train
DTM.train = removeSparseTerms(DTM.train, 0.9) #Removing anything uses less than 5% o the time considering the size of this DTM
DTM.train
...
# Create a logcistic regression model for each of the 50 authors
for(i in seq(1:50)){
  response = df[,i]
  #LASSO = glmnet(matrix.DTM.train, response, family='multinomial', alpha=1, lambda = .01)
  temp = cbind(response, as.data.frame(matrix.DTM.train))
  logit = glm(response~., data=temp, family=binomial(logit))
  assign(paste0(names[i],'_logit'),logit)
}
...
DTM.test = DocumentTermMatrix(my_corpus.test, control= list(weighting = function(x) weightTfIdf(x, normalize = TRUE)))
DTM.test
DTM.test = removeSparseTerms(DTM.test, 0.9) #Removing anything uses less than 5% o the time considering the size of this DTM
DTM.test
...
names = as.array(unique(labels))
df.test <- data.frame(matrix(ncol = 50, nrow = 2500)) #Initializing output DF
df.test[,] = 0
colnames(df.test) = names
location = 1
for(i in seq(1:50)){
  df.test[location:(location+49),i] = 1
  location = location  + 50
}

index = 1
df.final = data.frame(Document=rep(0,2500), Predict=rep(0,2500), Prob=rep(0,2500), Actual=rep(0,2500), Match=rep(0,2500))
df.final[,4] = labels
for(index in seq(1:51)){
  document = df.DTM.test[i,]
  current.best = 0
  current.name= NULL
  for(name in names){
    model.temp = get(paste0(name,'_logit'))
    predicted = predict(model.temp, as.data.frame(document), type='response')
    if(predicted > current.best){
      current.best = predicted
      current.name = name
    }
  ...
  df.final[index,2] = current.name #Best Prediction
  df.final[index,3] = current.best
